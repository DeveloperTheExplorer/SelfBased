...
if (isset($_POST['title'])){
	...
	if ($image == "na") {
		$sql = "UPDATE post SET title='$title', category='$category', web='$web', tags='$tags', blurb='$shortDesc', description='$longDesc', apple='$apple', android='$android', price='$price' WHERE id='$post_id' LIMIT 1";
		$query = mysqli_query($db_conx, $sql) or die(mysqli_error($db_conx));
	} else {
		$sql = "UPDATE post SET title='$title', category='$category', web='$web', tags='$tags', blurb='$shortDesc', description='$longDesc', image='$image', apple='$apple', android='$android', price='$price' WHERE id='$post_id' LIMIT 1";
		$query = mysqli_query($db_conx, $sql) or die(mysqli_error($db_conx));
	}
...
<head>
  <meta charset="UTF-8">
  <title>SelfBased - New Post</title>
			...
			<div class="parallax-user ">
				<div class="pageMiddle relative">
					...
					<div class="lg-dpn-background relative screen-right long-animation">
						...
						<textarea name="Large Description" id="post-lg-dpn"><?php echo $description ?></textarea>
					...
					<div class="absolute long-animation go-left screen-left top right full-width">
						...
						<div class="med-full-container small-margin-top setting-message">
							<div class="flex-box-between">
								...
								<div class="width-45">
									...
									<div class="border-lightgray small-padding-bottom">
										<div class="fancy-text-form fancy-login-form small-med-margin-top">
											<input type="text" maxlength="55" id="post-title" class="transparent-blue-box med-long-textbox post-required focusFancy" value="<?php echo $title; ?>"/>
											<label>Title*</lable>
										</div>

										<div class="fancy-text-form fancy-login-form small-med-margin-top">
											<select name="Category" id="post-category" class="transparent-blue-select med-long-textbox post-required focusFancy">
										...
										<div class="fancy-text-form fancy-login-form small-med-margin-top" data-tooltip="Example: https://example.com">
											<input type="text" id="post-web" class="transparent-blue-box med-long-textbox post-required focusFancy"  value="<?php echo $web; ?>" />
											<label>Link To Site*</lable>
										</div>

										<div class="fancy-text-form fancy-login-form small-med-margin-top" data-tooltip="Tags are used for searching. Seperate your tags by 'commas'.">
											<input type="text" maxlength="45" id="post-tag" class="transparent-blue-box med-long-textbox post-required focusFancy" value="<?php echo $tags; ?>" />
											<label>Tags*</lable>
										</div>

										<div class="fancy-text-form fancy-login-form small-med-margin-top">
											<label class="display-block italic">Blurb:* <em>(maximum: 550 characters)</em></label>
											<textarea name="Small Description" id="post-st-dpn" class="transparent-blue-textarea med-long-textbox contact-required post-required" placeholder="A Short Blurb About Your Fundraiser" maxlength="550"><?php echo $blurb; ?></textarea>
										</div>
									</div>
									...
									<div class="border-lightgray small-padding-bottom price-section">
										<div class="small-margin-top med-margin-left small-padding">
											<label class="inline cursor-pointer" for="post-free"><span class="checkboxSpan relative inline-block supertiny--margin-bottom short-animation <?php echo $price1; ?>"></span><span class="small-margin-left">Free</span></label>
											<input id="post-free" type="checkbox" class="transparent" <?php echo $price1; ?>><br/>
										</div>
										<div class="fancy-text-form fancy-login-form small-med-margin-top price-textbox" <?php echo $price2; ?>>
											<span><i class="fa fa-usd font-blue fa-lg"></i></span>
											<input type="text" id="post-price" class="transparent-blue-box med-long-textbox <?php echo $price3; ?>" value="<?php echo $price0; ?>"/>
									...
									<div class="border-lightgray small-padding-bottom" data-tooltip="If there are no apps, please leave the boxes empty.">
										<div class="fancy-text-form fancy-login-form small-med-margin-top">
											...
											<input type="text" maxlength="300" id="post-apple" class="transparent-blue-box med-long-textbox focusFancy" value="<?php echo $apple; ?>" />
											<label class="med-larger-margin-left">Apple App Url:</label>
										</div>

										<div class="fancy-text-form fancy-login-form small-med-margin-top">
											<span><i class="fa fa-android font-AndroidGreen fa-lg"></i></span>
											<input type="text" maxlength="300" id="post-android" class="transparent-blue-box med-long-textbox focusFancy" value="<?php echo $android; ?>" />
									...
									<div class="border-lightgray small-padding-bottom">
										<div class="fancy-text-form fancy-login-form small-med-margin-top" data-tooltip="Please select an image file under 2MB.">
											...
											<div class="cursor-pointer opacity-hover photo-upld" id="imgUpload" style="<?php echo $photo; ?>" onclick="triggerUpload(event, 'fu_SP')" data-postid="<?php echo $postid; ?>">
											</div>
										</div>
									</div>
								...
								<div class=" med-half-container small-margin-top small-padding">
									...
									<p class="font-gray">Please check to see if there are any apps available for the source you are adding!</p>
								</div>
						...
						<a href="user.php?u=<?php echo $log_username; ?>" class="med-btn gray-white-btn small-margin" > Cancel Edit </a>
	...
	$(function() {
      ...
      var $post_status = $('.post-status1');
		...
		$('.complete-post').click(function() {
			var $post_id = $('.photo-upld').attr('data-postid');
      ...
      var $post_price = $('#post-price').val();
			...
			if (full_story == "") {
				...
				$.post('edit_post.php', {
					...
					if(stage == "post_ok") {
						window.location = "source.php?id="+$post_id;
