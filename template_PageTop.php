...
if ($log_username != "") {
	...
	$feedlink = '
	<a href="feed.php">
		<li class="feed"><i class="fa fa-newspaper-o feed navicon"></i>Feed</li>
	</a>
	';
}
...
?><?php
	...
	if(isset($_POST["u"])){
	  ...
	  //   $query = mysqli_query($db_conx, $sql);
		...
		// END FORM DATA ERROR HANDLING
			...
			$sql = "INSERT INTO users (name, username, email, password, city, country, ip, signup, lastlogin, notescheck)
			        VALUES('$name','$user','$e','$p_hash','$city','$country','$user_ip',now(),now(),now())";
			...
			// $to = "$e";
			// $from = "auto_responder@SelfBased.com";
			// $subject = 'SelfBased Account Activation';
			// $message = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>SelfBased</title><style>a{text-decoration: none;color: #3EA7FD;}a:hover{color: #0073c0;text-decoration: underline;}h5{display: inline-block;margin-left: 20px;margin-top: 10px;color: #fff;}body{background-color: #EFEFEF;}</style></head><body style="margin:0px; background-color: #eee; font-family:Tahoma, Geneva, sans-serif;"> <div style="padding:10px; background:#0073C0; font-size:24px; color:#CCC;"><a href="https://www.SelfBased.com"><img src="https://www.SelfBased.com/style/logo.png" width="60" height="50" alt="SelfBased" style="border:none; float:left;"></a> <h5>SelfBased Activation</h5></div><div style="padding:24px; font-size:17px;">Hello '.$name.',<br /><br />Click the link below to activate your account when ready:<br /><br /><a href="https://www.SelfBased.com/activation.php?id='.$uid.'&u='.$user.'&e='.$e.'&p='.$p_hash.'">Click here to activate your account now</a><br /><br />After successful activation you can login using your::<br />* E-mail Address: <b>'.$e.'</b></div></body></html>';
			// $headers = "From: $from\n";
	    ...
	    // 	$query = mysqli_query($db_conx, $sql);
			...
			// 	$app = "Invite";
		  // 		$note = '<a href="user.php?u='.$user.'" style="color: #09F;"><img src="style/invited.png" alt="Donation" class="icon_donation" />'.$avatar_notif.'<img src="style/invited.png" alt="Donation" class="icon_donation flip" /><li class="Notification">Your friend '.$name.' just joined SelfBased. Check out '.$name.'\'s profile!</li></a>';
    ...
    if($subject != "" && $text != "") {
      ...
      $sql1 = "INSERT INTO feedback (username, subject, feedback, postdate)
			...
			$to = "feedback@selfbased.com";
      $from = "auto_responder@selfbased.com";
			$subject = "$subject";
			$message = 'This is a copy of a feedback from a user:
...
<div class="menu">
	...
	<a href="/" class="media-small1">
		<div class="logo-hover display-block">
			<img src="style/nosettles.svg" class="center" onerror="this.onerror=null; this.src=\'style/nosettles.png\'">
		</div>
	</a>
    ...
    <a href="https://www.facebook.com/SelfBased-457045167835524/?ref=hl" target="_blank"><i class="fa fa-facebook fa-lg icon"></i></a>
    <a href="https://twitter.com/SelfBased_Inc" target="_blank"><i class="fa fa-twitter fa-lg icon"></i></a>
    <a href="http://SelfBased.wordpress.com/" target="_blank"><i class="fa fa-rss fa-lg icon"></i></a>
	</div>
...
<header>
	    ...
	    <a href="/" class="media-small1">
		    	...
		    	<img src="style/nosettles.svg" id="logo" onerror="this.onerror=null; this.src=\'style/nosettles.png\'">
				</div>
	  	</a>
  	</div>
