...
if(isset($_GET['id'])) {
	...
	if($statusnumrows > 0){
	    ...
	    if($android != "") {
	      $android = '<a href="'.$android.'" target="_blank">Available</a>';
	    } else {
	      $android = "<span class='font-gray' >Not available</span>";
	    }
	    if($apple != "") {
	      $apple = '<a href="'.$apple.'" target="_blank">Available</a>';
	    } else {
	    ...
	    if($rating == 0) {
	      ...
	      $response = '<span class="font-blue">Excellent</span>';
			...
			if($author == $log_username) {
				...
				$edit = '<br><a href="edit_post.php?id='.$postid.'" class="font-green underline cursor-pointer">Edit Post</a>';
			}
			...
			if($statusnumrows > 0){
				while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
					...
					if ($commenter == $log_username) {
						$comments .= '<div class="med-padding comment" data-comment="'.$commentId.'"><span class="gray x delete-comment" style="font-weight: bold">X</span><div class="tiny-square profile_p inline-block" style="'.$comment_avatar.'"></div><a class="small-margin-left bold font-blue med-font" href="user.php?u='.$commenter.'">'.$comment_name.'</a><span class="small-font font-gray small-margin-left">'.$comment_date.'</span><br><div class="med-margin-left tiny-margin-top">'.$comment.'</div></div>';
					} else {
						$comments .= '<div class="med-padding comment" data-comment="'.$commentId.'"><div class="tiny-square profile_p inline-block" style="'.$comment_avatar.'"></div><a class="small-margin-left bold font-blue med-font" href="user.php?u='.$commenter.'">'.$comment_name.'</a><span class="small-font font-gray small-margin-left">'.$comment_date.'</span><br><div class="med-margin-left tiny-margin-top">'.$comment.'</div></div>';
					}
			...
			if($statusnumrows == 1) {
				...
				$comment_count = '<span class="comments"></span>';
			} else {
...
if(isset($_POST['comment_id']) && $_POST['comment_id'] != '') {
	...
	$comment_id = mysqli_real_escape_string($db_conx, $comment_id);
	$post_id = htmlentities($_POST['post_id']);
	$post_id = mysqli_real_escape_string($db_conx, $post_id);
	...
	if($comment_id == '') {
		...
		$sql = "SELECT * FROM comments WHERE postid='$post_id' ORDER BY postdate DESC LIMIT 999999999";
		$query = mysqli_query($db_conx, $sql);
		$statusnumrows = mysqli_num_rows($query);
		echo $statusnumrows;
		exit();
...
<body data-post="<?php echo $postid; ?>">
	...
	<main>
      ...
      <div class="profile-background border-top-bottom border-gray large-padding padding-top-bottom" data-author="<?php echo $author ?>" data-statusid="<?php echo $statusId; ?>" style="<?php echo $photo; ?>">
            ...
            <div class="flex-box">
              ...
              <div class="width--300 center-text">
                <h3>Apple App: <?php echo $apple; ?></h3>
              </div>
...
$(function() {
    $('#submitComment').click( function() {
      ...
      if(comment == '' || '<?php echo $log_username ?>' == '') {
        ...
        $.post('php_parsers/status_system.php', {
          ...
          if(data != "Fail") {
						...
						var numbers = datArray[0];
						var comment_id = datArray[1];
						if(numbers == 1) {
							$('.comments').html(numbers+" Comment");
						} else {
							$('.comments').html(numbers+" Comments");
						}
						$('#comment').val('');
						$('.no-comment').fadeOut(100);
            $('#new-comment').prepend('<div class="med-padding comment" data-comment="'+comment_id+'"><span class="gray x delete-comment" style="font-weight: bold">X</span><?php echo $picStyle1; ?><a class="small-margin-left bold font-blue med-font" href="user.php?u=<?php echo $log_username; ?>"><?php echo $name1; ?></a><span class="small-font font-gray small-margin-left">Just now</span><br><div class="med-margin-left tiny-margin-top">'+comment+'</div></div>');
          } else {
            alert('Sorry, there\'s a problem in the system. Please contact us, so we can fix it! Thank you!');
          }
        });
		...
		$('body').on('click', '.delete-comment', function() {
			...
			var the_comment = $(this);
			var post_id = $('body').attr('data-post');

			if(!$.isNumeric(comment_id)) {
				$('.comment-status').html('Oops, looks like something is wrong. Please contact us and let us know. Thanks for understanding!');
			} else {
				$.post('source.php', {
					...
					if(data != "failure") {
						...
						the_comment.parent().fadeOut('slow');
